            ...
            <template v-slot:top>
                <v-toolbar flat color="white">
                    ...
                    <v-dialog v-model="dialog" max-width="500px">
                        ...
                        <v-card>
                            <v-card-title>
                                <span class="headline">{{ editedItem.id ? 'Отредактировать' : 'Создать' }} категорию продукта</span>
                            </v-card-title>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            /**
             * Получить модели категории
             */
            getCategories() {
                this.getCategoriesVuex(this.params).then(res => {
                    this.items = res.data.data.map((i) => this.formatCat(i));
                })
            },
            /**
             * Функция-распределитель (сохранить или отредактировать категорию)
             */
            save() {
            ...
            createCategory() {
                ...
                return axios({
                    ...
                    if(res.data.success) {
                        this.items.push(this.formatCat(res.data.data));
                    }
                }).catch(e => {
            ...
            /**
             * Открыть модальное окно создания/редактирования категории
             * @param item
             */
            openModal(item) {
            ...
            /**
             * Закрыть модальное окно создания/редактирования категории
             */
            closeModal() {
                this.dialog = false;
